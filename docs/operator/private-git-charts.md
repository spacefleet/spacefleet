...
description: Let organizations deploy Helm charts from private Git repositories by registering a GitHub App for your Spacefleet deployment — creating the App, the permissions and Setup URL it needs, supplying its credentials safely as Helm values, how an organization connects an installation, and troubleshooting the connect flow. Covers the operator setup, not how to use the git chart source.
category: Operator
...
- **Setup URL** (in the *"Post installation"* section) —
  `https://<your external URL>/github/callback`, and enable **Redirect on
  update**. This is the one Spacefleet uses: GitHub sends the user here *after
  they install the App*, and Spacefleet records the installation. It must be your
  `config.externalURL` with `/github/callback` appended.
- **Callback URL** (in the *"Identifying and authorizing users"* section) —
  **leave it blank.** This is a *different* field, for GitHub's user-authorization
  (OAuth) flow, which Spacefleet does not use. Despite the matching name, the
  `/github/callback` URL above goes in **Setup URL**, not here.
...
Create the App, then collect three things:
...
**The feature is enabled only when all three are set.** With any empty,
Spacefleet treats the GitHub App as not configured and the Git chart source
remains public-only.

> [!IMPORTANT]
> The `/github/callback` address goes in the App's **Setup URL** field (the
> *"Post installation"* section) — **not** the **Callback URL** field (the
> *"Identifying and authorizing users"* section), which Spacefleet doesn't use
> and you should leave blank. The Setup URL must be `config.externalURL` +
> `/github/callback`. If a user finishes installing on GitHub and lands on a "not
> found" page or the connection silently fails, a wrong/empty Setup URL (or the
> URL placed in the Callback URL field by mistake) is the usual cause.

## Configure it

Provide the App ID and slug inline and the private key as a secret. A minimal
setup:
...
`config.github.privateKey` is a credential — keep it out of plaintext values and
Git history. It accepts the PEM either as-is (multi-line) or base64-encoded
(single line, friendlier for env vars). You have two ways to supply it, exactly
like the SMTP password and the credential-encryption key (see
[Secret configuration](secrets.md)):
...
**From a Secret you manage (recommended for production / GitOps).** Put the key
in your own Secret under the key `GITHUB_APP_PRIVATE_KEY` and reference it — your
...
single Secret can carry `GITHUB_APP_PRIVATE_KEY` alongside
`SPACEFLEET_SECRET_KEY` and the database URL. If you set the key both inline and
via `envFrom`, the inline value wins.
...
2. GitHub returns them to Spacefleet, which records the installation. The
   handshake is tied to the organization that started it, so an installation
   can't be claimed by a different organization.
...
```sh
kubectl exec deploy/<release>-web    -- printenv GITHUB_APP_ID GITHUB_APP_SLUG
kubectl exec deploy/<release>-worker -- printenv GITHUB_APP_ID GITHUB_APP_SLUG
```
...
- **Lands on a "not found" page after installing, or the connection fails** —
  the App's **Setup URL** doesn't match `config.externalURL` + `/github/callback`.
  Fix it on the App's GitHub settings page.
- **"github app is not configured" (the button is missing)** — one of
  `appId`/`slug`/`privateKey` is empty, or didn't reach the pods. Re-check the
  three values and the `printenv` above.
