...
// Nodes is the Infrastructure › Nodes page. It lists the Kubernetes nodes of the
// organization's registered clusters — by default every node across every
// connected cluster, filterable to a single cluster — and updates live: each
// node's row links to a detailed view. Nodes are streamed per cluster over SSE
// (see lib/useClusterNodeStreams), so the table reflects changes without a
...
export function Nodes() {
  ...
  // Clusters page); only the nodes within them stream live.
  ...
  // connected cluster (an unconnected cluster can't serve nodes).
  const targets = useMemo(() => {
    if (filter !== ALL) return clusters.filter((c) => c.id === filter);
    return clusters.filter((c) => c.status === "connected");
  }, [clusters, filter]);

  const { rows, status, errors } = useClusterNodeStreams(targets);
  ...
  // connected — surfaced so an empty table isn't mistaken for "no nodes".
  const skipped =
            ...
            <tbody>
              {sortedRows.map((n) => (
                <tr
                  ...
                  onClick={() =>
                    navigate(
                      `/infrastructure/nodes/${n.clusterId}/${encodeURIComponent(n.name)}`,
                    )
                  }
                ...
                >
                  ...
                  <td className="px-4 py-3 text-gray-600">{nodeAge(n.created_at)}</td>
                </tr>
              ))}
            </tbody>
          </table>
  ...
  return (
    <div className="p-10 text-center">
      ...
      <p className="mt-1 text-sm text-gray-500">
        Register a Kubernetes cluster to see its nodes here.
      </p>
      <Link
        to="/admin/clusters"
...
function NoNodes({ skipped }: { skipped: number }) {
  return (
    <div className="p-10 text-center">
      <Boxes className="mx-auto h-8 w-8 text-gray-300" />
      <p className="mt-3 text-sm font-medium text-gray-700">No nodes to show</p>
      <p className="mt-1 text-sm text-gray-500">
        {skipped > 0
          ...
          : "This cluster reported no nodes."}
      </p>
    </div>
  );
}
