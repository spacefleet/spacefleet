...
func (_q *DeploymentQuery) QueryApplication() *ApplicationQuery {
...
func (_q *DeploymentQuery) WithApplication(opts ...func(*ApplicationQuery)) *DeploymentQuery {
	query := (&ApplicationClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(deployment.FieldOrganizationID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Deployment.Query().
//		Select(deployment.FieldOrganizationID).
//		Scan(ctx, &v)
...
func (_q *DeploymentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Deployment, error) {
	...
	if query := _q.withApplication; query != nil {
		if err := _q.loadApplication(ctx, query, nodes, nil,
			func(n *Deployment, e *Application) { n.Edges.Application = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *DeploymentQuery) loadApplication(ctx context.Context, query *ApplicationQuery, nodes []*Deployment, init func(*Deployment), assign func(*Deployment, *Application)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Deployment)
	for i := range nodes {
...
func (_q *DeploymentQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withOrganization != nil {
			_spec.Node.AddColumnOnce(deployment.FieldOrganizationID)
		}
		if _q.withApplication != nil {
			_spec.Node.AddColumnOnce(deployment.FieldApplicationID)
		}
	}
