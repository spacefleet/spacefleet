...
func (_q *WorkflowRunQuery) QueryOrganization() *OrganizationQuery {
...
func (_q *WorkflowRunQuery) QueryApplication() *ApplicationQuery {
...
func (_q *WorkflowRunQuery) WithOrganization(opts ...func(*OrganizationQuery)) *WorkflowRunQuery {
	query := (&OrganizationClient{config: _q.config}).Query()
...
func (_q *WorkflowRunQuery) WithApplication(opts ...func(*ApplicationQuery)) *WorkflowRunQuery {
	query := (&ApplicationClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(workflowrun.FieldOrganizationID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.WorkflowRun.Query().
//		Select(workflowrun.FieldOrganizationID).
//		Scan(ctx, &v)
...
func (_q *WorkflowRunQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*WorkflowRun, error) {
	...
	if query := _q.withOrganization; query != nil {
		if err := _q.loadOrganization(ctx, query, nodes, nil,
			func(n *WorkflowRun, e *Organization) { n.Edges.Organization = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withApplication; query != nil {
		if err := _q.loadApplication(ctx, query, nodes, nil,
			func(n *WorkflowRun, e *Application) { n.Edges.Application = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *WorkflowRunQuery) loadOrganization(ctx context.Context, query *OrganizationQuery, nodes []*WorkflowRun, init func(*WorkflowRun), assign func(*WorkflowRun, *Organization)) error {
...
func (_q *WorkflowRunQuery) loadApplication(ctx context.Context, query *ApplicationQuery, nodes []*WorkflowRun, init func(*WorkflowRun), assign func(*WorkflowRun, *Application)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*WorkflowRun)
	for i := range nodes {
...
func (_q *WorkflowRunQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withOrganization != nil {
			_spec.Node.AddColumnOnce(workflowrun.FieldOrganizationID)
		}
		if _q.withApplication != nil {
			_spec.Node.AddColumnOnce(workflowrun.FieldApplicationID)
		}
	}
