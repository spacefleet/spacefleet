...
func (_c *UserCreate) check() error {
	...
	if v, ok := _c.mutation.OidcSubject(); ok {
		if err := user.OidcSubjectValidator(v); err != nil {
			return &ValidationError{Name: "oidc_subject", err: fmt.Errorf(`ent: validator failed for field "User.oidc_subject": %w`, err)}
		}
	}
...
func (_c *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec) {
	...
	if nodes := _c.mutation.OrganizationsIDs(); len(nodes) > 0 {
		...
		createE := &MembershipCreate{config: _c.config, mutation: newMembershipMutation(_c.config, OpCreate)}
		createE.defaults()
...
func (u *UserUpsertOne) UpdateNewValues() *UserUpsertOne {
	...
	u.create.conflict = append(u.create.conflict, sql.ResolveWith(func(s *sql.UpdateSet) {
		...
		if _, exists := u.create.mutation.OidcSubject(); exists {
			s.SetIgnore(user.FieldOidcSubject)
		}
...
func (u *UserUpsertBulk) UpdateNewValues() *UserUpsertBulk {
	...
	u.create.conflict = append(u.create.conflict, sql.ResolveWith(func(s *sql.UpdateSet) {
		for _, b := range u.create.builders {
			...
			if _, exists := b.mutation.OidcSubject(); exists {
				s.SetIgnore(user.FieldOidcSubject)
			}
